...
%         xy
    ...
    methods (Static)
        ...
        function [idx, min_dist] = mindist(xy,pt)
            % [idx, min_dist] = mindist(xy,pt)
            % Return point from xy cloud with minimum distance to pt

            N = size(xy,2);
            xy_pt = xy - repmat( pt, 1, N );

            dist2 = sqrt( sum( xy_pt.^2, 1 ) );

            [min_dist,idx] = min( dist2 );
        end
        
        function d = dist( l, xy )
            % d = dist( l, xy )
            % Input:
            %   l  - homogeneous line in scan plane
            %   xy - 2xN array of 2D points in scan plane
            % Output:
            %   d  - 1xN vector of signed distances of xy points to l line
            
            % Normalize line
            l   = l / norm(l(1:2));
            xyw = makehomogeneous( xy );
            
            d = l' * xyw;
        end
        
        % For representation
        function h = plotLRFframe( orientationLabel )
            
            % Plot axis labels
            hl(1) = xlabel('X');
            hl(2) = ylabel('Y');
            
            % Plot LRF representation as a marker
            if ~exist('orientationLabel','var')
                orientationLabel = 'right';
            end
            switch orientationLabel
                case 'right'
                    hm = plot(0,0,'>', 'MarkerSize',10, 'LineWidth',3);
                case 'up'
                    hm = plot(0,0,'^', 'MarkerSize',10, 'LineWidth',3);
            end
            ...
            axis equal;
            % Plot LRF main axes
            ha(1) = plotHomLineWin( [1 0 0], 'k' );
            ha(2) = plotHomLineWin( [0 1 0], 'k' );
            
            % Put all together for output
            h = [hl, hm, ha];
        end
        function h = plotXY( xy, format )
            
            h = plot( xy(1,:),xy(2,:), format );
        end
    end
end
