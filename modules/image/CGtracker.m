...
%             plot(this.segs.pts(1,:), this.segs.pts(2,:), '*r');
        ...
        function [new_seg, pts] = optimSeg( this, seg, pts )
            % [seg, pts] = optimSeg( this, seg, pts )
            % Optimize segment from a window a pixels in object image
            % Input:
            %   * this - Contains image data: image, grad image, ...
            %   seg - the initial estimate of segment
            %   pts - the location of interest pixels (window)
            
            % Create output variable segment (independent different of input object)
            new_seg = copy(seg);
            
            % Get indeces of pixels from array of points
            X = pts(1,:)'; Y = pts(2,:)';
            ...
            mask_dir = this.filterDir(pts, new_seg.v, pi/16);
            % Fuse both masks
            mask = mask_mag & mask_dir;
            
            % Use gradient magnitude to weigh line adjustment
            w = mag';
...
%             end
            ...
            l1 = [ new_seg.v; -new_seg.v'*new_seg.p1 ];
            new_seg.p1 = makeinhomogeneous( cross( l, l1 ) );
            l2 = [ new_seg.v; -new_seg.v'*new_seg.p2 ];
            new_seg.p2 = makeinhomogeneous( cross( l, l2 ) );
...
%             end
            ...
            doTrim = false;
            if doTrim % Remove points with distance to segment center below certain percentile (likely to be outliers)
                % Get distance of points on new line
                m = [new_seg.v; -new_seg.v'*new_seg.c];
...
%                 figure
                this.plotGdir( pts(:,mask), new_seg );
                this.plotSeg( new_seg );
                keyboard
...
%                 figure
        ...
        function [pts, corners] = segmentWindow( this, seg, h, g )
            % pts = segmentWindow( seg )
            % pts = segmentWindow( seg, h, g )
            % [pts, corners] = segmentWindow( seg, h, g )
            % Get pixels inside orthogonal distance from segment (window
            % around segment)
            % Input:
            %   seg - the reference segment
            %   h - half-width of window
            %   g - length increase of window wrt segment
            % Output:
            %   pts - location of pixels inside window
