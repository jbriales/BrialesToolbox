classdef CSegment2D < handle & matlab.mixin.Copyable
    ...
    properties(Dependent)
        ...
        l % Homogeneous line
        d % Segment length
        dv% Direction vector with segment length
    end
    
    methods
        function this = CSegment2D(p1,p2,d)
            % this = CSegment2D(p1,p2)
            % this = CSegment2D(c,v,d)
            if nargin ~= 0 % Allow nargin == 0 syntax
                if nargin == 3 % Case center, direction and length
                    % Correct input meaning
                    c = p1;
                    v = p2;
                    
                    p1 = c - v * d/2;
                    p2 = c + v * d/2;
                end % End of multisyntax correction
        ...
        % Overload useful functions
        function out = isempty( this )
            out = ( isempty(this.p1) && isempty(this.p2) );
        end
        ...
        function A_l = get.A_l( this )
            ...
            if isempty(this.A_l) && ~isempty(this.p1)
        ...
        function project2line( this, c, v )
            % project2line( this, c, v )
            % This method corrects current segment so that p1 and p2
            % belong to the line defined by central point c and direction v
            % Input:
            %   c - 2x1 central point in the new line
            %   v - 2x1 normal direction of the new line
            % Output:
            %   Updated segment 2D with p1 and p2 on the line
            
            % Find new points as orthogonal projection in the new line
            n = ort_2D(v); % Orthogonal direction
            l = [ n; -n'*c ]; % New line (homogeneous)
            m1 = [ v; -v'*this.p1 ]; % Line on p1 orthogonal to l
            m2 = [ v; -v'*this.p2 ]; % Line on p2 orthogonal to l
            this.p1 = makeinhomogeneous( cross(m1,l) );
            this.p2 = makeinhomogeneous( cross(m2,l) );
        end
